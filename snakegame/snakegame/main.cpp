...
int mode = 0;			//游戏模式
int score = 0;			//分数
unsigned int t1, t2;			//定义两个时间控制移动速度
int map[ROW][COL];		//地图大小
COORD snake[1024];		//蛇			typedef struct _COORD {SHORT XSHORT Y} COORD, *PCOORD
unsigned int SnakeSize;		//蛇的尺度		typedef unsigned int     size_t
char SnakeDir;			//蛇移动方向
//需要的函数
void start();	//开始界面
...
void start()
{
	...
	//规则设定
	outtextxy(290, 80, (LPCTSTR)"功能选择:\n");
	outtextxy(280, 150, (LPCTSTR)"1.普通模式\n");
	outtextxy(280, 200, (LPCTSTR)"2.穿墙模式\n");
	outtextxy(280, 250, (LPCTSTR)"3.无敌模式\n");
	outtextxy(200, 280, (LPCTSTR)"数字键 1,2,3 选择模式，Enter键进入游戏\n");
	outtextxy(200, 300, (LPCTSTR)"字母键 W,S,A,D 方向键 上下左右 控制方向\n");
...
void DrawMap()
{
	...
	cleardevice();		//清屏
	char arr[10];		//保存成绩
	wsprintf(arr, "总分：%d", score);	//将成绩格式化输出到字符串arr中 
